...
class Array
  ...
  def dominant_octupus(&prc)
    prc ||= Proc.new { |x, y| x <=> y }

    return self if count <= 1

    midpoint = count / 2
    sorted_left = self.take(midpoint).dominant_octupus(&prc)
    sorted_right = self.drop(midpoint).dominant_octupus(&prc)
...
p @fish.dominant_octupus { |fish1, fish2| fish2.length <=> fish1.length }[0]
